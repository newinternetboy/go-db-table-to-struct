...
func (t *Table2Struct) Run() error {
	...
	for tableRealName, item := range tableColumns {
		...
		default:
			// 字符长度大于1时
			tableNameSplits := strings.Split(tableName, "_")
			var camelTableName string
			for _, v := range tableNameSplits {
				camelTableName += strings.ToUpper(v[0:1]) + strings.ToLower(v[1:])
			}
